...
- The public surface centers on three orchestration entry points —
  ...
  (`StorageSCP`, `StoreSession`, `AssociationPool`, `DicomCache`) and
  standalone converter/multipart utilities, all re-exported from
  `__init__.py`. If a use case genuinely isn't covered by any of them,
  **extend** `__init__.py`'s exports — add a method/function to a
  private module and re-export it — rather than importing the private
  module directly.
